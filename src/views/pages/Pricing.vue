<template>
  <v-container id="pricing" class="fill-height justify-center" tag="section">
    <v-row align="center" class="text-center" justify="center">
      <v-col class="mb-10" cols="12">
        ...
        <div class="display-1 font-weight-light grey--text text--lighten-2">
          You have Free Unlimited Updates and Premium Support on&nbsp;<br
      ...
      <template v-for="(plan, i) in plans">
        <v-col :key="i" class="d-flex" cols="12" sm="6" md="3">
          <pages-plan-card :plan="plan" />
        </v-col>

        <v-col
          v-if="
            i === 1 && $vuetify.breakpoint.smAndDown && i + 1 !== plans.length
          "
          :key="`divider-${i}`"
          class="pa-6"
          cols="12"
